function __fzf_cd_subdir -d "Change directory without replacing current commandline."
    # Make sure that fzf uses fish to execute __fzf_preview_file.
    # See similar comment in __fzf_search_shell_variables.fish.
    set --local --export SHELL (command --search fish)

    set fd_opts --color=always -t d $fzf_fd_opts
    ...
    if test $status -eq 0
        ...
        if test "$commandline_tokens" = "$current_token"
            ...
            builtin cd "$file_paths_selected"
        end
    end
